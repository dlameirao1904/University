...
void setup(){
    ...
    login = new Login();
	  ...
	  j2 = new Jogador();
    keys = new boolean[4];
    c = new Cliente("172.26.12.186", 12345);
...
void desenhaJogo() {
  String estadoS;
  ArrayList<Bola> bolasCopy = new ArrayList<>(bolas);  // Cria uma cópia da lista bolas
  ...
  synchronized (bolas) {
    bolasCopy = new ArrayList<>(bolas);
  }
  synchronized (bolas) {
  if (bolasCopy != null) {
  for (Bola bola : bolasCopy) {
    bola.desenha();
  }}
  }
}
...
void string2estado(String e){
    ...
    float x, y, tam, dir ,velAng, aceLin ;
    int t;
  ...
  if (estado == 1) {
    menu.topScore = new String[4];
    for (int i = 0; i < 4; i++) {
        if (scanner.hasNext()) {
            menu.topScore[i] = scanner.next();
        }
    }
    scanner.close();
    return;
    ...
    j1.setMassa(tam);
    scanner.next();
    dir = scanner.nextFloat();
    j1.setDirecao(dir);

    velAng = scanner.nextFloat();
    j1.setVelAng(velAng);

    aceLin = scanner.nextFloat();
    j1.setAceLin(aceLin);
