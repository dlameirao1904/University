class Menu{
	...
	public String topScore [] = new String[4];
	public String msg = "";
    ...
    fill(0);
		...
		text("Minha pontuação: ", width/8, height/8);
    line++;
    text(">>  " + topScore[0], width/8, height/8+ line * 40);
		line++;
		text("Top Jogadores:", width/8, height/8+ line * 40);
		for (int i = 1; i < topScore.length; i++){
			line++;
			text(i + "º " + topScore[i], width/6, height/8 + line * 40);
		}
		line+=2;
	...
	void addTopScore(String[] topScore){
